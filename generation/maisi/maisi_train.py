...
trained_g_path = os.path.join(args.model_dir, f"train_autoencoder_{exp}.pt")
trained_d_path = os.path.join(args.model_dir, f"train_discriminator_{exp}.pt")
...
for epoch in range(start_epoch, max_epochs):
    ...
    if epoch % val_interval == 0:
        ...
        if val_loss_g < best_val_recon_epoch_loss:
            ...
            trained_g_path_epoch = f"train_{trained_g_path[:-3]}_epoch{epoch}.pt"
            trained_d_path_epoch = f"train_{trained_d_path[:-3]}_epoch{epoch}.pt"
            ...
            print("Save trained discriminator to", trained_d_path_epoch)
